...
const dataDummy: ChartDataProps[] = [
...
const plugins = {
  ...
  afterDraw: (chart: any, args: any, options: any) => {
    ...
    fillTextMultiLine(
      ...
      totalHeight - (labelBStart + labelBEnd) * step
    );
    fillTextMultiLine(
      ctx,
      labelC,
      chart.chartArea.right + 10,
      totalHeight - (labelCStart + labelCEnd) * step - 8
    );
    fillTextMultiLine(
      ctx,
      labelD,
      chart.chartArea.right + 10,
      totalHeight - (labelDStart + labelDEnd) * step
    );
    fillTextMultiLine(
      ctx,
      labelE,
      chart.chartArea.right + 10,
      totalHeight - (labelEStart + labelEEnd) * step
    );
    fillTextMultiLine(
      ctx,
      labelF,
      chart.chartArea.right + 10,
      totalHeight - (labelFStart + labelFEnd) * step
    );
  },
};
...
function ChartComponent(props: { data: ChartDataProps[] }) {
  const { data } = props;
  const rData = data.reverse();
  const xLabels = rData.map((d) => d.date);
  const yValues = rData.map((d) => d.value);
